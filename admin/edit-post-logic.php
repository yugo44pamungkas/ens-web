...
if (isset($_POST['submit'])) {
    ...
    // set is_featured to 0 if was unchecked 
    ...
    if (!$title) {
        ...
        // delete existing thumbnail if now thumbnail is available
        if ($thumbnail['name']) {
            ...
            // Rename Image
            $time = time(); // make each images name upload unique using current timestamp
    ...
    // redirect back (with form data) to add-post ppage if there is any problem
    ...
    if (!mysqli_errno($connection)) {
        $_SESSION['edit-post-success'] = "Post added successfully";
    }
}
